...
def collect_scores(root_dir, sub_folder, tag):
    # 定义输出文件路径：在目标文件夹根目录下
    output_file_name = f"all_{tag}_score.txt"
    output_path = os.path.join(root_dir, sub_folder)
    ...
    for current_root, dirs, files in os.walk(root_dir):
        if os.path.basename(current_root) == sub_folder:
            if tag == "best_50":
            ...
            if file_name in files:
                ...
                try:
                    ...
                    # 2. 处理文件夹名称
                    folder_name = os.path.basename(os.path.dirname(current_root))
...
def process_directory(folder_path, sub_folder):
...
if __name__ == "__main__":
    ...
    parser.add_argument("sub_folder", type=str, help="包含各级子文件夹的目标目录路径 (例如 result)")

    # 解析参数
    args = parser.parse_args()
    
    target_dir = args.folder
    target_dir = os.path.join("results", target_dir)
    sub_folder = f"eval_{args.sub_folder}"


    # 检查路径有效性
    if os.path.exists(target_dir) and os.path.isdir(target_dir):
        collect_scores(target_dir, sub_folder, "final")
        collect_scores(target_dir, sub_folder, "best")
        collect_scores(target_dir, sub_folder, "best_50")
        process_directory(target_dir, sub_folder)
    else:
