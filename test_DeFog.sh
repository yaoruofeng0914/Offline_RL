...
LOG_DIR="eval_logs"
SUMMARY_FILE="DeFog_Summary_Scores.csv"
...
# 写入 CSV 表头
echo "Environment,Seed,Noise_Type,Attack_Type,DeFog_Att,DeFog_Raw" > "$SUMMARY_FILE"
# 1. 统计总任务数（应用所有过滤条件）
echo "正在统计任务总量..."
TOTAL_TASKS=0
...
for env_dir in "$BASE_DIR"/*; do
    ...
    for condition_dir in "$env_dir"/*; do
        ...
        # 独立日志文件
        LOG_FILE="${LOG_DIR}/DeFog_${env}_${ATTACK_MODE}_${location}_seed${SEED}.log"
