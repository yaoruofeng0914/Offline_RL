...
BASE_DIR=~/Offline_RL/checkpoint4baseline/CQL
LOG_DIR="eval_logs"
SUMMARY_FILE="CQL_Summary_Scores.csv"
...
# 写入 CSV 表头
echo "Environment,Seed,Noise_Type,Attack_Type,CQL_Att,CQL_Raw" > "$SUMMARY_FILE"
...
for env_dir in "$BASE_DIR"/*; do
    ...
    for condition_dir in "$env_dir"/*; do
        ...
        remainder="${condition_name#$"CQL"_${env}_}"
...
for env_dir in "$BASE_DIR"/*; do
    ...
    for condition_dir in "$env_dir"/*; do
        ...
        # 独立日志文件
        LOG_FILE="${LOG_DIR}/CQL_${env}_${ATTACK_MODE}_${location}_seed${SEED}.log"
