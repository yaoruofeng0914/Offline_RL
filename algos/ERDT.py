...
def compute_loss(config, model, batch, current_epoch=None):
    log_dict, debug_dict = {}, {}
    ...
    # ED-RDT: 能量惩罚项（鼓励DT生成低能量动作）
    if hasattr(model, 'domain_energy') and model.domain_energy is not None:
        if not torch.isnan(predicted_actions).any():
            state_flat = states.reshape(-1, config.state_dim)
            action_flat = predicted_actions.reshape(-1, config.action_dim)
            e_domain = model.domain_energy(state_flat, action_flat)
            # 即使不使用，也记录下 e_behavior 观察是否依然崩溃
            e_behavior = model.behavior_energy(state_flat, action_flat)

            # 【关键修改】：总能量等于纯粹的域能量
            # total_energy = e_domain
            total_energy = e_domain + 0.3 * e_behavior
            total_energy = total_energy.reshape(states.shape[0], states.shape[1])

            # 监控能量分布标度
            log_dict["energy/domain_mean"] = e_domain.mean().item()
            log_dict["energy/behavior_mean"] = e_behavior.mean().item()

            # Top-K 相对惩罚
            K = 0.2
            threshold = torch.quantile(total_energy, 1 - K)
            energy_penalty = F.relu(total_energy - threshold).mean()

            # 动态预热：前 50% epoch 线性增长系数，最高到 10.0
            if current_epoch is not None:
                warmup_ratio = min(1.0, current_epoch / (0.5 * config.num_epochs + 1e-6))
            else:
                warmup_ratio = 1.0
            penalty_coef = 1 * warmup_ratio

            loss += penalty_coef * energy_penalty
            log_dict["energy_penalty"] = energy_penalty.item()
            log_dict["energy/penalty_coef"] = penalty_coef
    data_info = None
...
def train(config: TrainConfig, logger: Logger):
    ...
    for epoch in trange(1, config.num_epochs + 1, desc="Training"):
        ...
        for step in trange(config.num_updates_on_epoch, desc="Epoch", leave=False):
            ...
            optim.zero_grad()
            loss, log_dict, debug_dict, data_info = compute_loss(config, model, batch,current_epoch=epoch)
            loss.backward()
