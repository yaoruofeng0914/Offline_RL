...
def ratio_dataset(h5path, output_path, env_name, ratio):
    random.seed(1234)
    np.random.seed(1234)

    h5path = os.path.expanduser(f"{h5path}/{env_name}.hdf5")
    ...
    output_path = os.path.join(output_path, f"{env_name}_ratio_{ratio}.pt")
    torch.save(new_dataset, output_path)
    print(f"Save downsample dataset in {output_path}")
    print(f"=============================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--h5path", type=str, default="/home/user/Offline_RL/original")
    parser.add_argument("--output_path", type=str, default="/home/user/Offline_RL/datasets")
    args = parser.parse_args()
    ratio_dataset(args.h5path, args.output_path, args.env_name, args.ratio)
