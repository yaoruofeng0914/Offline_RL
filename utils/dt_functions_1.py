...
class SequenceDataset:
    def __init__(self, config, logger: Logger = None):
        ...
        # 训练能量函数（用于 CEG 引导）
        self.domain_energy, self.behavior_energy = train_energy_functions(
            self.dataset, config.state_dim, config.action_dim, logger, config.device
        )
        ...
        if model.use_asts and not model.training:
            ...
            if trigger_asts:
                with torch.enable_grad():
                    ...
                    rdt_action = predicted_actions[:, -1, :]  # [1, action_dim]
                    current_state = states[:, step]  # [1, state_dim]

                    # 使用 CEG 能量引导替代 ASTS 硬截断
                    if hasattr(model, 'domain_energy') and model.domain_energy is not None:
                        guided_action = sgld_guided_action(
                            rdt_action, current_state,
                            model.domain_energy, model.behavior_energy,
                            num_steps=5, step_size=0.02, noise_scale=0.003,
                            domain_weight=1.0, behavior_weight=0.3
                        )
                        predicted_actions[:, -1, :] = guided_action
        with torch.no_grad():
            predicted = model(
                states[:, : step + 1][:, -model.seq_len:],
                actions[:, : step + 1][:, -model.seq_len:],
            ...
            # 🌟 CEG 能量引导（替代 ASTS 硬截断）
            if trigger_asts and hasattr(model, 'domain_energy') and model.domain_energy is not None:
                with torch.enable_grad():
                    rdt_action = predicted_actions[:, -1, :]  # [1, action_dim]
                    current_state = states[:, step]  # [1, state_dim]
                    guided_action = sgld_guided_action(
...
# ==========================================
# 🌟 Idea 4: 跨域能量引导的生成式序列鲁棒控制 (CEG-Robust-DT)
# ==========================================

class DomainEnergyFunction(nn.Module):
    """域能量函数：低能量代表"干净数据流形"，高能量代表"偏离流形"（可能是攻击）"""
...
class BehaviorEnergyFunction(nn.Module):
    """行为能量函数：低能量代表"高价值行为"，高能量代表"低价值行为" """
    ...
    def forward(self, state, action):
        # 负号：高Q值 → 低能量（引导策略向高价值区域移动）
        return -self.net(torch.cat([state, action], dim=-1)).squeeze(-1)


def train_energy_functions(dataset, state_dim, action_dim, logger, device="cpu"):
    """训练域能量函数和行为能量函数"""
    logger.info("开始训练能量函数...")
    ...
    # 1. 训练域能量函数
    domain_energy = DomainEnergyFunction(state_dim, action_dim, 256).to(device)
    optim_domain = torch.optim.Adam(domain_energy.parameters(), lr=1e-4)

    for epoch in range(50):
        noise_states = states + torch.randn_like(states) * 0.1
        noise_actions = actions + torch.randn_like(actions) * 0.1

        energy_clean = domain_energy(states, actions)
        energy_noise = domain_energy(noise_states, noise_actions)

        # NCE 损失：拉开干净样本和噪声样本的能量差距
        loss_domain = F.relu(1.0 - (energy_noise - energy_clean)).mean()
        loss_domain += 0.01 * (energy_clean ** 2 + energy_noise ** 2).mean()

        optim_domain.zero_grad()
        loss_domain.backward()
        optim_domain.step()

    logger.info(f"域能量函数训练完成，最终 loss: {loss_domain.item():.4f}")

    # 2. 训练行为能量函数
    behavior_energy = BehaviorEnergyFunction(state_dim, action_dim, 256).to(device)
    optim_behavior = torch.optim.Adam(behavior_energy.parameters(), lr=1e-4)
    ...
    for traj in dataset:
        ...
        all_dones.append(torch.tensor([0] * (len(traj["rewards"]) - 1) + [1], dtype=torch.float32))
    ...
    for t in reversed(range(len(rewards))):
        running_return = rewards[t] + 0.99 * (1 - dones[t]) * running_return
        mc_returns[t] = running_return

    mc_returns = (mc_returns - mc_returns.mean()) / (mc_returns.std() + 1e-6)

    for epoch in range(100):
        pred_values = -behavior_energy(states, actions)
        loss_behavior = F.mse_loss(pred_values, mc_returns)

        optim_behavior.zero_grad()
        loss_behavior.backward()
        optim_behavior.step()

    logger.info(f"行为能量函数训练完成，最终 loss: {loss_behavior.item():.4f}")

    return domain_energy, behavior_energy


def sgld_guided_action(
        rdt_action, state, domain_energy, behavior_energy,
        num_steps=5, step_size=0.02, noise_scale=0.003,
        domain_weight=1.0, behavior_weight=0.3
):
    """使用 SGLD 对动作进行能量引导优化"""
    action = rdt_action.clone().detach().requires_grad_(True)

    for _ in range(num_steps):
        if action.grad is not None:
            action.grad.zero_()

        e_domain = domain_energy(state, action)
        e_behavior = behavior_energy(state, action)
        total_energy = domain_weight * e_domain + behavior_weight * e_behavior

        grad = torch.autograd.grad(total_energy.sum(), action)[0]

        noise = torch.randn_like(action) * noise_scale
        action = action - step_size * grad + noise
