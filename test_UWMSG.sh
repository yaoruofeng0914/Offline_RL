...
BASE_DIR=~/Offline_RL/checkpoint4baseline/UWMSG
LOG_DIR="eval_logs"
SUMMARY_FILE="UWMSG_Summary_Scores.csv"
...
# 写入 CSV 表头
echo "Environment,Seed,Noise_Type,Attack_Type,UWMSG_Att,UWMSG_Raw" > "$SUMMARY_FILE"
...
for env_dir in "$BASE_DIR"/*; do
    ...
    for condition_dir in "$env_dir"/*; do
        ...
        remainder="${condition_name#$"UWMSG"_${env}_}"
...
for env_dir in "$BASE_DIR"/*; do
    ...
    for condition_dir in "$env_dir"/*; do
        ...
        # 独立日志文件
        LOG_FILE="${LOG_DIR}/UWMSG_${env}_${ATTACK_MODE}_${location}_seed${SEED}.log"
